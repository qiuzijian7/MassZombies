...
UClass* UComboGraphSchema::GetRuntimeClassForAnimAsset(const UAnimationAsset* Asset, const UEdGraph* Graph)
{
	UComboGraph* ComboGraph = Cast<UComboGraph>(Graph->GetOuter());

	if (Asset->GetClass()->IsChildOf(UAnimMontage::StaticClass()))
	{
		return ComboGraph ? ComboGraph->DefaultNodeMontageType : UComboGraphNodeMontage::StaticClass();
	}

	if (Asset->GetClass()->IsChildOf(UAnimSequence::StaticClass()))
	{
		return ComboGraph ? ComboGraph->DefaultNodeSequenceType : UComboGraphNodeSequence::StaticClass();
	}
