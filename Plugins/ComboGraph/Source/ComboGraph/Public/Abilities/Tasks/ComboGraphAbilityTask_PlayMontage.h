...
struct FComboGraphMontageNotifyAutoSetupData
{
	...
	UPROPERTY()
	UAnimNotifyState* CachedNotify = nullptr;
...
private:
	...
	UPROPERTY(Transient)
	TArray<UAnimNotifyState*> RegisteredNotifyStates;

	UPROPERTY(Transient)
	TMap<UClass*, FComboGraphRegisteredNotifyData> RegisteredNotifyData;
	...
	FAnimNotifyEvent* GetAnimNotify(UAnimSequenceBase* AnimSequenceBase, UClass* NotifyClass);
	bool HasAnimNotify(UAnimMontage* Montage, UClass* NotifyClass);
};
