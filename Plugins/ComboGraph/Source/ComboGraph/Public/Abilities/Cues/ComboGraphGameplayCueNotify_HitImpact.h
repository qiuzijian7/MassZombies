...
protected:
	...
	void AsyncLoadAssets(TArray<FSoftObjectPath> AssetsToLoad, FStreamableDelegate DoneDelegate = FStreamableDelegate()) const;
	void AsyncLoadAsset(const FSoftObjectPath& AssetToLoad, FStreamableDelegate DoneDelegate = FStreamableDelegate()) const;
	...
	void GetSpawnLocationAndRotationFromParameters(AActor* MyTarget, const FGameplayCueParameters& Parameters, FVector& Location, FRotator& Rotation);

	/** Extract location and rotation information from Hit Result */
	void GetSpawnLocationAndRotationFromHitResult(const FHitResult* HitResult, FVector& Location, FRotator& Rotation);
};
