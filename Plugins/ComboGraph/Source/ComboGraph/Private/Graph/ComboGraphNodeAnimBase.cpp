...
UClass* UComboGraphNodeAnimBase::K2_GetAnimationClass() const
{
	...
	return AnimationAsset ? AnimationAsset->GetClass() : nullptr;
}
...
{
	return Cast<UComboGraphAbilityTask_StartGraph>(CachedOwningTask);
}

UGameplayAbility* UComboGraphNodeAnimBase::K2_GetOwningAbility() const
{
	return CachedOwningAbility;
}

UComboGraphNodeAnimBase* UComboGraphNodeAnimBase::K2_GetPreviousNode() const
{
	if (const UComboGraphAbilityTask_StartGraph* Task = Cast<UComboGraphAbilityTask_StartGraph>(CachedOwningTask))
	{
...
FGameplayCueParameters UComboGraphNodeAnimBase::CreateGameplayCueParameters(const FGameplayEventData& EventData) const
{
	...
	AActor* Instigator = EventData.Instigator ? Cast<AActor>(EventData.Instigator) : nullptr;
