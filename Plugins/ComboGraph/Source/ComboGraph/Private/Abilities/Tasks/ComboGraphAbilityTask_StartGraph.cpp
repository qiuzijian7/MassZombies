...
void UComboGraphAbilityTask_StartGraph::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(UComboGraphAbilityTask_StartGraph, CurrentInputActionInstance);
}
...
bool UComboGraphAbilityTask_StartGraph::AdvanceNextNode(UComboGraphNodeAnimBase* NextNode, FString& FailReason)
{
	...
	CG_RUNTIME_LOG(Verbose, TEXT("AdvanceNextNode: %s"), *GetNameSafe(NextNode), *GetNameSafe(NextNode->GetClass()))
...
bool UComboGraphAbilityTask_StartGraph::CheckCostForNode(UComboGraphNodeAnimBase* Node, FString& FailReason)
{
	...
	UGameplayEffect* CostGE = GetCostGameplayEffectForNode(Node);
	const FGameplayAbilityActorInfo* ActorInfo = Ability->GetCurrentActorInfo();
...
void UComboGraphAbilityTask_StartGraph::SetupInputBindings()
{
	...
	for (UComboGraphNodeBase* ChildNode : CurrentNode->ChildrenNodes)
	{
		...
		if (Edge)
		{
			// This is to ensure edges are unique per task so that owning actor passed in when setting input binding is not shared
			// (workaround for 4.26 EnhancedInput not allowing to pass additional arguments in BindAction callbacks, to know which InputAction it is referring to)
			UComboGraphEdge* NewEdge = DuplicateObject(Edge, this);
			if (NewEdge && NewEdge->TransitionInput)
			{
				const UInputAction* InputAction = NewEdge->TransitionInput;

				NewEdge->SetOwningTask(this);
				NewEdge->SetOwningActor(GetAvatarActorFromActorInfo());

				// Convert out internal enum to the real Input Trigger Event for Enhanced Input
				const ETriggerEvent TriggerEvent = NewEdge->GetEnhancedInputTriggerEvent();
				...
				uint32 ConfirmHandle = InputComponent->BindAction(InputAction, TriggerEvent, NewEdge, &UComboGraphEdge::ReceivedInputConfirm).GetHandle();
...
void UComboGraphAbilityTask_StartGraph::OnEventInputReceived(const FGameplayEventData EventData)
{
	...
	EComboGraphTransitionInputEvent TriggerEvent = InputEdge ? InputEdge->TriggerEvent : EComboGraphTransitionInputEvent::Triggered;
	...
	for (UComboGraphNodeBase* ChildNode : CurrentNode->ChildrenNodes)
	{
		...
		{
			UComboGraphEdge* Edge = CurrentNode->GetEdge(ChildAnimNode);
			...
			{
				CG_RUNTIME_LOG(Log, TEXT("OnEventInputReceived: Handle input confirm for edge and tag %s (Child: %s)"), *Tag.ToString(), *ChildAnimNode->GetName())
				if (InputEdge)
				{
...
void UComboGraphAbilityTask_StartGraph::HandleInputConfirmed(UComboGraphNodeAnimBase* NextNode, UComboGraphEdge* Edge)
{
	check(Ability);

	UAnimMontage* Montage = Ability->GetCurrentMontage();
	if (!Montage)
	{
		CG_RUNTIME_LOG(Error, TEXT("HandleInputConfirmed: GetCurrentMontage() returns invalid Montage"))
		return;
	}
...
FComboGraphGameplayEffectContainerSpec UComboGraphAbilityTask_StartGraph::MakeEffectContainerSpecFromContainer(const FComboGraphGameplayEffectContainer& Container, FGameplayTag ContainerTag, const FGameplayEventData& EventData, int32 OverrideGameplayLevel)
{
	...
	UAbilitySystemComponent* OwningASC = Ability->GetAbilitySystemComponentFromActorInfo();
...
{
	FComboGraphGameplayEffectContainer* FoundContainer = EffectContainerMap.Find(ContainerTag);
	if (FoundContainer)
	{
...
void UComboGraphAbilityTask_StartGraph::ApplyDamageContainer(UComboGraphNodeAnimBase* ComboNode, FGameplayTag EventTag, FGameplayEventData EventData) const
{
	...
	if (FoundContainer)
	{
		...
		{
			DamageCauser = Cast<AActor>(EventData.Instigator);
		}
...
float UComboGraphAbilityTask_StartGraph::GetNodeMagnitudeForContainer(UComboGraphNodeAnimBase* ComboNode, const float OriginalMagnitude, const FGameplayTag ContainerTag) const
{
	check(ComboNode);
