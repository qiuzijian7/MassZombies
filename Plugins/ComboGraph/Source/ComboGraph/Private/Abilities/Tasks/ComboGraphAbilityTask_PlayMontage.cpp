...
FString UComboGraphAbilityTask_PlayMontage::GetDebugString() const
{
	...
	if (Ability)
	{
		if (CachedAnimInstance != nullptr)
		{
			PlayingMontage = CachedAnimInstance->Montage_IsActive(MontageToPlay) ? MontageToPlay : CachedAnimInstance->GetCurrentActiveMontage();
		}
	}

	return FString::Printf(TEXT("UComboGraphAbilityTask_PlayMontage. MontageToPlay: %s  (Currently Playing): %s"), *GetNameSafe(MontageToPlay), *GetNameSafe(PlayingMontage));
}
...
void UComboGraphAbilityTask_PlayMontage::TickTask(const float DeltaTime)
{
	...
	if (CurrentSection != CachedCurrentSection)
	{
		...
		if (CachedCurrentSection != NAME_None)
		{
			...
			TMap<UClass*, FComboGraphRegisteredNotifyData> NewRegisteredNotifyData;
...
void UComboGraphAbilityTask_PlayMontage::TickHandleNotifyState(USkeletalMeshComponent* SkeletalMeshComponent, const float SequenceLength, UAnimNotifyState* NotifyState)
{
	...
	if (CurrentSectionElapsedTime >= StartTime && !NotifyData->bBeginNotified)
	{
		...
		CG_RUNTIME_LOG(Log, TEXT("UComboGraphAbilityTask_PlayMontage - TickTask Should Trigger NotifyBegin - ElapsedTime >= StartTime - %f >= %f (%f)"), CurrentSectionElapsedTime, StartTime, SequenceLength);
		CG_RUNTIME_LOG(Log, TEXT("UComboGraphAbilityTask_PlayMontage - TickTask, Trigger notify begin on %s with skel mesh: %s"), *NotifyState->GetName(), SkeletalMeshComponent ? *SkeletalMeshComponent->GetName() : TEXT("NONE"));
	...
	else if (CurrentSectionElapsedTime >= EndTime && !NotifyData->bEndNotified)
	{
		...
		CG_RUNTIME_LOG(Log, TEXT("UComboGraphAbilityTask_PlayMontage - TickTask Should Trigger NotifyEnd - ElapsedTime >= StartTime - %f >= %f (%f)"), CurrentSectionElapsedTime, EndTime, SequenceLength);
		CG_RUNTIME_LOG(Log, TEXT("UComboGraphAbilityTask_PlayMontage - TickTask, Trigger notify end on %s with skel mesh: %s"), *NotifyState->GetName(), SkeletalMeshComponent ? *SkeletalMeshComponent->GetName() : TEXT("NONE"));
...
{
	for (const auto AutoSetup : NotifiesAutoSetup)
	{
...
bool UComboGraphAbilityTask_PlayMontage::HasAnimNotify(UAnimMontage* Montage, UClass* NotifyClass)
{
	...
	for (FSlotAnimationTrack SlotAnimTrack : Montage->SlotAnimTracks)
	{
		for (const FAnimSegment AnimSegment : SlotAnimTrack.AnimTrack.AnimSegments)
		{
			...
			FAnimNotifyEvent* NotifyEvent = GetAnimNotify(AnimSegment.AnimReference, NotifyClass);
			if (NotifyEvent)
			{
				return true;
			}
	...
	FAnimNotifyEvent* NotifyEvent = GetAnimNotify(Montage, NotifyClass);
